...
class TurtlebotControl(Node):
    ...
    def control_loop(self):
        if self._logger:
            self.get_logger().info('Posição atual x: ' + self.position.x + ', Posição atual y: ' + self.position.y )
            self.get_logger().info('Posição final x: ' + self.x + ', Posição final y: ' + self.y )
            self._logger = False
        ...
        self.angle_ajust(self, self.angulo)
